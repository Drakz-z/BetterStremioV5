module.exports = class WatchParty {
  ...
  getName = () => "WatchParty";
  getImage = () =>
    "https://raw.githubusercontent.com/Drakz-z/BetterStremioV5/main/WatchParty.logo.png";
  getDescription = () =>
  ...
  getVersion = () => "1.2.2";
  getAuthor = () => "Drakz-z";
  getShareURL = () => "https://github.com/Drakz-z/BetterStremioV5";
  getUpdateURL = () =>
    "https://raw.githubusercontent.com/Drakz-z/BetterStremioV5/main/WatchParty.plugin.js";
  onBoot = () => {
    const currentServices = window.services;
    if (currentServices) {
      window.__WatchPartyServices = currentServices;
      return;
    }
    ...
    Object.defineProperty(window, "services", {
      ...
      set(value) {
        servicesValue = value;
        window.__WatchPartyServices = value;
      },
    });
  };
  onReady = () => {};
  ...
  onDisable() {
    ...
    clearTimeout(WatchParty.idleTimer);
    if (WatchParty.resetIdleTimer) {
        document.removeEventListener("mousemove", WatchParty.resetIdleTimer);
        document.removeEventListener("mousedown", WatchParty.resetIdleTimer);
        document.removeEventListener("keydown", WatchParty.resetIdleTimer);
    }
    document.removeEventListener("click", WatchParty.outsideClickListener);
    window.removeEventListener("hashchange", WatchParty.hashChangeListener);
    WatchParty.openUI(false);
    document.getElementById("wp-control")?.remove();
    document.getElementById("wp-popup")?.remove();
    document.getElementById("wp-chat-container")?.remove();
    ...
    delete window.__WatchPartyServices;
  ...
  onLoad() {
    ...
    const WatchPartyControl = document.createElement(
      windowControls ? "li" : "div",
    );
    WatchPartyControl.id = "wp-control";
    WatchPartyControl.innerHTML = this.control();
    if (!windowControls) {
      WatchPartyControl.classList.add("wp-floating");
    }
    ...
    if (!windowControls) {
      WatchPartyPopup.classList.add("wp-v5");
    }
    ...
    if (windowControls) {
      ...
      document.body.appendChild(WatchPartyControl);
      document.body.appendChild(WatchPartyPopup);
    }
    ...
    WatchParty.copyText = this.copyText;
    WatchParty.isV5 = () => this.isV5();
    WatchParty.getCurrentHash = () => this.getCurrentHash();
    WatchParty.getVideoApi = () => this.getVideoApi();
    WatchParty.getPlaybackState = () => this.getPlaybackState();
    WatchParty.leave = () =>
    ...
    window.WatchParty.isIdle = false;
    WatchParty.chatObserver = setInterval(() => {
      ...
      const container = document.getElementById("wp-chat-container");
      if (container) {
        ...
        const isChatOpen = chat && chat.style.display !== "none";
        const rect = container.getBoundingClientRect();
        const isHovered = window.WatchParty.mouseX >= rect.left && window.WatchParty.mouseX <= rect.right &&
                          window.WatchParty.mouseY >= rect.top && window.WatchParty.mouseY <= rect.bottom;
        
        let shouldShow = false;
        if (isChatOpen) {
            shouldShow = isHovered;
        } else {
            shouldShow = !WatchParty.isIdle || isHovered;
        }
        
        if (hideChat) shouldShow = false;
        
        container.style.opacity = shouldShow ? "1" : "0";
        const toggle = document.getElementById("wp-chat-toggle");
        if (toggle) toggle.style.pointerEvents = shouldShow ? "auto" : "none";
        if (chat) chat.style.pointerEvents = shouldShow ? "auto" : "none";
      }
    }, 400);
    WatchParty.playerObserverPaused = null;
    WatchParty.playerObserverSpeed = null;
    WatchParty.playerObserverTime = null;
    WatchParty.playerObserverTick = Date.now();
    WatchParty.lastBroadcastHash = WatchParty.getCurrentHash();
    WatchParty.playerObserver = setInterval(() => {
      const WatchParty = window.WatchParty;
      const BetterStremio = window.BetterStremio;
      if (WatchParty.isV5()) {
        const state = WatchParty.getPlaybackState();
        if (!state) return;
        const now = Date.now();
        const expectedDelta = WatchParty.playerObserverPaused === false
          ? (now - WatchParty.playerObserverTick) *
            (WatchParty.playerObserverSpeed ?? 1)
          : 0;
        const actualDelta = WatchParty.playerObserverTime === null
          ? 0
          : state.time - WatchParty.playerObserverTime;
        const seekDetected = WatchParty.playerObserverTime !== null &&
          Math.abs(actualDelta - expectedDelta) > 1500;

        if (
          WatchParty.playerObserverPaused !== state.paused ||
          WatchParty.playerObserverSpeed !== state.playbackSpeed ||
          seekDetected
        ) {
          WatchParty.broadcast("state", {
            ...state,
            force: true,
            hash: WatchParty.getCurrentHash(),
          });
          WatchParty.playerObserverPaused = state.paused;
          WatchParty.playerObserverSpeed = state.playbackSpeed;
        }
        WatchParty.playerObserverTime = state.time;
        WatchParty.playerObserverTick = now;
        return;
      }
    ...
    window.WatchParty.toggleChat = () => {
        const chat = document.getElementById("wp-chat");
        if (chat) chat.style.display = chat.style.display === "none" ? "flex" : "none";
    };

    window.WatchParty.mouseX = 0;
    window.WatchParty.mouseY = 0;
    WatchParty.resetIdleTimer = (e) => {
        if (e && e.type === "mousemove") {
            window.WatchParty.mouseX = e.clientX;
            window.WatchParty.mouseY = e.clientY;
            if (e.movementX === 0 && e.movementY === 0) return;
        }
        WatchParty.isIdle = false;
        clearTimeout(WatchParty.idleTimer);
        const container = document.getElementById("wp-chat-container");
        const rect = container ? container.getBoundingClientRect() : null;
        const isHovered = rect && window.WatchParty.mouseX >= rect.left && window.WatchParty.mouseX <= rect.right &&
                          window.WatchParty.mouseY >= rect.top && window.WatchParty.mouseY <= rect.bottom;
        if (isHovered) {
            WatchParty.idleTimer = setTimeout(WatchParty.resetIdleTimer, 3500);
        } else {
            WatchParty.idleTimer = setTimeout(() => {
                WatchParty.isIdle = true;
            }, 3500); // 3.5s matches Stremio's typical idle timeout
        }
    };
    document.addEventListener("mousemove", WatchParty.resetIdleTimer);
    document.addEventListener("mousedown", WatchParty.resetIdleTimer);
    document.addEventListener("keydown", WatchParty.resetIdleTimer);
    ...
    if (WatchParty.isV5()) {
      WatchParty.hashChangeListener = () => WatchParty.inject();
      window.addEventListener("hashchange", WatchParty.hashChangeListener);
    } else if (!BetterStremio.StremioRoot.wpInjectedStateChange) {
  ...
  isV5() {
    return !!(window.__WatchPartyServices || window.services) &&
      !window.BetterStremio?.Modules?.$state;
  }
  ...
  isPlayerHash(hash = this.getCurrentHash()) {
    return /^#\/player\/[^/]+/i.test(this.normalizeHash(hash));
  }
  ...
  findPlayerFiber() {
    ...
    const stack = nodes
      .map((node) => this.getReactFiber(node))
      .filter(Boolean);
  ...
  getVideoApi() {
    const WatchParty = window.WatchParty;
    if (!WatchParty || !this.isV5()) return null;

    const cached = WatchParty.v5VideoApi;
    if (cached?.videoRef?.current && cached?.containerRef?.current) {
      return cached.api;
    }

    const playerFiber = this.findPlayerFiber();
    if (!playerFiber) return null;

    let hook = playerFiber.memoizedState;
    let videoRef = null;
    ...
    while (hook) {
      ...
      if (
        state?.current &&
        typeof state.current.dispatch === "function" &&
        typeof state.current.destroy === "function"
      ) {
        videoRef = state;
      } else if (
        state?.current &&
        typeof state.current.nodeType === "number"
      ) {
        containerRef = state;
      } else if (
        state &&
        typeof state === "object" &&
        ["paused", "time", "duration", "playbackSpeed"].every((key) =>
    ...
    if (!videoRef?.current || !containerRef?.current || !stateRef) return null;
    ...
    const api = {
      get state() {
        return stateRef.memoizedState || {};
      },
      setPaused(paused) {
        dispatch({ type: "setProp", propName: "paused", propValue: paused });
      },
      setTime(time) {
      ...
      on: (...args) => videoRef.current.on?.(...args),
      off: (...args) => videoRef.current.off?.(...args),
    };

    WatchParty.v5VideoApi = { api, containerRef, stateRef, videoRef };
    return api;
  }
  ...
  getPlaybackState() {
    ...
    if (WatchParty.isV5()) {
      const api = WatchParty.getVideoApi();
      if (!api?.state || api.state.paused === null || api.state.time === null) {
        return null;
      }
      return {
        state: { state: api.state.paused ? 2 : 4 },
        time: api.state.time,
        paused: api.state.paused,
        playbackSpeed: api.state.playbackSpeed ?? 1,
      };
    }

    const BetterStremio = window.BetterStremio;
    if (!BetterStremio?.Player) return null;
    return {
      state: BetterStremio.Player.state,
      time: BetterStremio.Player.time,
  ...
  inject() {
    ...
    if (!WatchParty) return;
    if (WatchParty.isV5()) {
      WatchParty.getVideoApi();
      const currentHash = WatchParty.getCurrentHash();
      const previousHash = WatchParty.lastBroadcastHash;

      if (WatchParty.NO_SPREAD_NAVIGATION) {
        WatchParty.lastBroadcastHash = currentHash;
        return;
      }

      if (currentHash !== previousHash) {
        if (this.isPlayerHash(currentHash)) {
          WatchParty.broadcast("go", { hash: currentHash });
        } else if (this.isPlayerHash(previousHash)) {
          WatchParty.broadcast("goback", { hash: currentHash });
        }
        WatchParty.lastBroadcastHash = currentHash;
      }
      return;
  ...
  execCommand(latency, cmd, data) {
    ...
    if (WatchParty.isV5()) {
      if (cmd === "go") {
        const nextHash = this.normalizeHash(data?.hash || data);
        WatchParty.NO_SPREAD_NAVIGATION = true;
        WatchParty.lastBroadcastHash = nextHash;
        window.location.replace(nextHash);
        setTimeout(() => delete WatchParty.NO_SPREAD_NAVIGATION, 250);
        return;
      } else if (cmd === "goback") {
        WatchParty.NO_SPREAD_NAVIGATION = true;
        window.history.back();
        setTimeout(() => delete WatchParty.NO_SPREAD_NAVIGATION, 250);
        return;
      } else if (cmd === "state" && !WatchParty.NO_SPREAD) {
        const video = WatchParty.getVideoApi();
        if (!video?.state) return;

        WatchParty.NO_SPREAD = true;
        if (data.playbackSpeed && video.state.playbackSpeed !== data.playbackSpeed) {
          video.setPlaybackSpeed(data.playbackSpeed);
        }

        if (video.state.paused !== data.paused && data.force) {
          video.setPaused(data.paused);
        }

        const maxDelay = (WatchParty.maxDelayBetweenSync + parseInt(latency)) *
          ((data.playbackSpeed ?? 1) + 0.1);

        if (Math.abs(data.time - video.state.time) > maxDelay) {
          video.setTime(data.time + parseInt(latency));
          if (video.state.paused !== data.paused) {
            video.setPaused(data.paused);
          }
        }
  ...
  chat() {
    ...
    <style type="text/css" id="wp-chat-style">
      ...
      #wp-chat-container:hover #wp-chat {
      ...
      #wp-chat-container:hover #wp-chat > #wp-msgs {
    ...
    <div id="wp-chat-container">
      <div id="wp-chat-toggle" onclick="window.WatchParty.toggleChat()">
        <svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor"><path d="M20 2H4c-1.1 0-1.99.9-1.99 2L2 22l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zM6 9h12v2H6V9zm8 5H6v-2h8v2zm4-6H6V6h12v2z"/></svg>
      </div>
  ...
  message(username, isHost, isSystem, text) {
    ...
    if (document.querySelector("#wp-chat-container:hover")) return;
  ...
  popup() {
    ...
    <div id="wp-inparty" class="tab" style="margin-top: 10px;">
      ...
      <h3 class="row" onclick="window.WatchParty.copyText(window.WatchParty.code())" style="cursor: pointer; color: gray;justify-content: flex-start;margin-bottom: 10px;">
    ...
    <style type="text/css">
      ...
      #wp-control.wp-floating {
        ...
        color: white;
        cursor: pointer;
        box-shadow: rgba(0, 0, 0, 0.35) 0px 12px 30px;
        z-index: 999999;
        list-style: none;
      }

      #wp-control.wp-floating svg {
        width: 28px;
        height: 28px;
      }
